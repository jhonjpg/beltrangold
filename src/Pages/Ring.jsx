...
const Ring = () => {
  ...
  const ringData = t('BeltranCo.ring', { returnObjects: true });
  const productCount = ringData.length;

  return (
    <section className="w-full flex flex-col">
      <div className="portadR w-full h-96 flex items-center justify-center">
        <h3 className="text-5xl text-white text-yellow-300">{t('Ring')}</h3>
      </div>
      <div className="w-full p-8 mt-20 flex justify-between">
        <h4 className="w-2/3">
          Sort by: <select> <option value="feature">{t('Feature')}</option> </select>
        </h4>
      ...
      <ul className="flex w-full h-96 flex-wrap gap-3 justify-center">
        {ringData.map((ring) => (
          <li key={ring.id} className="w-2/5 h-52 flex flex-col justify-between gap-3 p-1 mb-3 md:w-1/4 h-80">
            <Link to={`/beltrangold/ring/${ring.name}`} className="w-full h-72 overflow-hidden">
