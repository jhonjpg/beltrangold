...
const Chain = () => {
    ...
    const productCount = es.BeltranCo.chain.length; // Calculate the number of products
...
<h3 className="text-5xl text-white text-yellow-300">chain</h3>
   ...
   <h4 className="w-2/3"> Sort by: <select> <option value="feature">Feature</option>  
    ...
    <ul className="flex w-full flex-wrap gap-3 justify-center ">
      {es.BeltranCo.chain.map((chain) => (
        ...
        <Link to={`/BeltranJewelry/chain/${chain.name}`} className="w-full h-72 overflow-hidden">
